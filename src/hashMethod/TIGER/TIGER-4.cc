...
std::string TIGER4_MAIN(uint8_t *message, size_t messageLength, size_t digestSize)
{
    //Where to add size
    ...
    for (size_t offset = 0; offset < ((paddedMessageLength) / 8); offset++)
    {
        ...
        //Switch a->c->b->a bcs after 4th round they will not be at the right place
        temp = a,
        a = c,
        c = b,
        b = temp;
        ...
        //Pass
        TRIGER_ROUND(a, b, c, W[0], 7);
        TRIGER_ROUND(b, c, a, W[1], 7);
        TRIGER_ROUND(c, a, b, W[2], 7);
        TRIGER_ROUND(a, b, c, W[3], 7);
        TRIGER_ROUND(b, c, a, W[4], 7);
        TRIGER_ROUND(c, a, b, W[5], 7);
        TRIGER_ROUND(a, b, c, W[6], 7);
        TRIGER_ROUND(b, c, a, W[7], 7);
        ...
        //Pass
        TRIGER_ROUND(a, b, c, W[0], 9);
        TRIGER_ROUND(b, c, a, W[1], 9);
        TRIGER_ROUND(c, a, b, W[2], 9);
        TRIGER_ROUND(a, b, c, W[3], 9);
        TRIGER_ROUND(b, c, a, W[4], 9);
        TRIGER_ROUND(c, a, b, W[5], 9);
        TRIGER_ROUND(a, b, c, W[6], 9);
        TRIGER_ROUND(b, c, a, W[7], 9);
...
Napi::Value TIGER4_128(const Napi::CallbackInfo &info)
{
    ...
    Napi::String num = Napi::String::New(env, TIGER4_MAIN((uint8_t *)(arg0.c_str()), arg0.length(), 128));
...
Napi::Value TIGER4_160(const Napi::CallbackInfo &info)
{
    ...
    std::string arg0 = info[0].As<Napi::String>();
    Napi::String num = Napi::String::New(env, TIGER4_MAIN((uint8_t *)(arg0.c_str()), arg0.length(), 160));
    return reinterpret_cast<Napi::Value &&>(num);
}
Napi::Value TIGER4_192(const Napi::CallbackInfo &info)
{
    ...
    std::string arg0 = info[0].As<Napi::String>();
    Napi::String num = Napi::String::New(env, TIGER4_MAIN((uint8_t *)(arg0.c_str()), arg0.length(), 192));
    return reinterpret_cast<Napi::Value &&>(num);
}