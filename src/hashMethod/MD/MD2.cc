...
#include <iomanip>
#include <sstream>
const uint8_t pi[] = {0x29, 0x2E, 0x43, 0xC9, 0xA2, 0xD8, 0x7C, 0x01, 0x3D, 0x36, 0x54, 0xA1, 0xEC, 0xF0, 0x06, 0x13,
...
std::string MD2_MAIN(const uint8_t *message, const size_t messageLength)
{
    ...
    for (int i = messageLength; i < newMessageLength; i++)
    ...
    for (int offset = 0; offset < (newMessageLength / 16); offset++)
    {
        for (int j = 0; j < 16; j++)
        {
    ...
    for (int offset = 0; offset < ((paddedMessageLength) / 16); offset++)
    {
        for (int j = 0; j < 16; j++)
        {
